...
if config['model_name'] in ['LSP']:
    model = LSP(model_name=config['model_name'], latent_size=config['latent_size'],
                ...
                gpu=config['device']).to(config['device'])
...
def train():
    ...
    for epoch in range(start_epoch+1, config['epochs']):
        model.train()
        loss_all_dict = {'all': 0, 'recon': 0., 'dis': 0., 'dir': 0., 'kl': 0.}
        ...
        for iter, sample in enumerate(trainDataLoader, 0):
            ...
            if global_iter % 1 == 0:
                print('Epoch[%3d], iter[%3d]: loss=[%.4f], recon=[%.4f], dir=[%.4f], dis=[%.4f], kl=[%.4f]' \
                        % (epoch, iter, loss.item(), loss_recon.item(), loss_dir.item(), loss_dis.item(), loss_kl.item()))
...
def evaluate(phase='val', set='val', save_res=True, info='batch'):
    ...
    loss_all_dict = {'all': 0, 'recon': 0., 'dir': 0., 'dis': 0., 'kl': 0.}
    img1_list = []
    img2_list = []
    label_list = []
    recon1_list = []
